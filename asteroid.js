...
const LASER_DIST = 0.2; //max distance laser can travel as fraction of screen width
...
function newAsteroid(x, y, r) {
  var roid = {
    x: x,
    y: y,
    xv: ((Math.random() * ROIDS_SPD) / FPS) * (Math.random() < 0.5 ? 1 : -1), //speed of asteroids
...
function update() {
  ...
  if (!exploding) {
    if (blinkOn) {
      (ctx.strokeStyle = "white"), (ctx.lineWidth = Ship_size / 10);
      ctx.beginPath();
      ctx.moveTo(
